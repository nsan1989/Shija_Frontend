          ...
          <Nav className={styles.navItemsWrapperStyles}>
            {MegaMenu.map((item, index) => (
              <div key={index} className={styles.megaMenuWrapper}>
                {/* Top-level menu */}
                {item.type === "logo" ? (
                  <Link to="/" className={`d-none d-md-flex ${styles["logoWrapper"]}`}>
                    <Image
                      src={logo}
                      alt="Shija Logo"
                      width={80}
                ...
                {item.firstSubMenu && (
                  <div
                    className={`${styles.megaMenuFrame} ${
                      activeMegaIndex === index ? styles.megaActive : ""
                    }`}
                    onMouseEnter={() => {
                      if (!isMobile) {
                        clearTimeout(closeTimer.current);
                  ...
                  >
                    ...
                    {(!isMobile && activeFirstIndex === null
                      ? item.firstSubMenu[0]
                      : item.firstSubMenu[activeFirstIndex]
                    )?.secondSubMenu && (
                      <div className={styles.secondSubMenu}>
                        {isMobile && activeFirstIndex !== null && (
                          <div
                            className={styles.backHeader}
                            onClick={() => setActiveFirstIndex(null)}
                          >
                            <FaArrowLeft />
                            <span>
                              {item.firstSubMenu[activeFirstIndex]?.name}
                            </span>
                          </div>
                        )}
                        {(
